    ...
    <>
      ...
      <div className="layout">
        ...
        <aside className={`sidebar ${showMenu?'open':''}`}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:28}}><h1 style={{fontSize:22,fontWeight:700}}>SellSync</h1><span style={{cursor:'pointer'}} className="hamburger" onClick={()=>setShowMenu(false)}>✕</span></div>
          {menu.map(m=><div key={m.id} onClick={()=>{setActive(m.id); setShowMenu(false)}} className="menuItem" style={{background:active===m.id?'#1e293b':'transparent',color:active===m.id?'white':'#94a3b8'}}><span>{m.icon}</span>{m.id}</div>)}
        </aside>

        <main className="main">
          <div className="topbar">
            <div style={{display:'flex',alignItems:'center',gap:10}}><span className="hamburger" onClick={()=>setShowMenu(true)}>☰</span><div style={{fontWeight:700}}>{active}</div></div>
            <div className="search hide-m"><span>🔍</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." /></div>
            <div style={{display:'flex',gap:14,alignItems:'center'}}>
              <div style={{position:'relative'}}><div onClick={()=>setShowNotif(!showNotif)} style={{cursor:'pointer'}}>🔔</div>{showNotif && <div style={{position:'absolute',right:0,top:36,width:260,background:'white',border:'1px solid #e2e8f0',borderRadius:12,padding:12,fontSize:12}}><b>Notifications</b><div style={{marginTop:8}}>⚠️ Dettol low</div></div>}</div>
              <img src="https://i.pravatar.cc/100?img=33" style={{width:32,height:32,borderRadius:'50%'}}/>
            </div>
          </div>

          {/* MOBILE SEARCH */}
          <div style={{padding:'12px 16px',background:'white',borderBottom:'1px solid #e2e8f0'}} className="show-m-only"><div className="search" style={{maxWidth:'100%'}}><span>🔍</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search products..." /></div></div>

          {active==='Dashboard' && (
            <>
              <div className="cards">
                <div className="card" onClick={()=>setActive('Sales')}><div style={{fontSize:11,color:'#64748b'}}>TOTAL SALES</div><div style={{fontSize:22,fontWeight:700,margin:'8px 0'}}>₦1,245,000</div><span style={{background:'#dcfce7',color:'#15803d',padding:'4px 8px',borderRadius:20,fontSize:11}}>+14.2% this week</span></div>
                <div className="card" onClick={()=>setActive('Inventory')}><div style={{fontSize:11,color:'#64748b'}}>PRODUCTS</div><div style={{fontSize:22,fontWeight:700,margin:'8px 0'}}>328</div><span style={{background:'#e0e7ff',color:'#4338ca',padding:'4px 8px',borderRadius:20,fontSize:11}}>18 categories</span></div>
                <div className="card" onClick={()=>setActive('Inventory')}><div style={{fontSize:11,color:'#64748b'}}>LOW STOCK</div><div style={{fontSize:22,fontWeight:700,margin:'8px 0'}}>12</div><span style={{background:'#fee2e2',color:'#b91c1c',padding:'4px 8px',borderRadius:20,fontSize:11}}>Needs restock</span></div>
                <div className="card" onClick={()=>setActive('Sales')}><div style={{fontSize:11,color:'#64748b'}}>TODAY'S TRANSACTIONS</div><div style={{fontSize:22,fontWeight:700,margin:'8px 0'}}>86</div><span style={{background:'#f1f5f9',padding:'4px 8px',borderRadius:20,fontSize:11}}>₦184k today</span></div>
              </div>
              <div className="grid2">
                ...
                <div className="box" style={{background:'#0f172a',color:'white'}}><h3 style={{marginBottom:8}}>✨ AI Insight</h3><p style={{fontSize:13,lineHeight:1.6,color:'#cbd5e1'}}>Peak Milk is selling faster than usual this week. Consider restocking within the next 2 days to avoid running out of stock.</p><button onClick={()=>showToast('Restock order created')} style={{marginTop:16,background:'white',color:'#0f172a',border:0,padding:'8px 14px',borderRadius:8,fontSize:12,cursor:'pointer',fontWeight:600}}>Create Restock Order</button></div>
              </div>
              <div style={{padding:'0 16px 16px'}}><div className="box"><h3 style={{marginBottom:12}}>Inventory Table</h3><table><thead><tr><th>Product Name</th><th>Category</th><th>Stock Qty</th><th>Price</th><th>Stock Status</th></tr></thead><tbody>{filtered.map(item=>{const c=getColor(item.stock); return <tr key={item.id}><td style={{fontWeight:600}}>{item.product}</td><td>{item.category}</td><td>{item.stock}</td><td>₦{item.price.toLocaleString()}</td><td><span style={{background:c.bg,color:c.c,padding:'4px 10px',borderRadius:20,fontSize:11,fontWeight:600}}>{getStatus(item.stock)}</span></td></tr>})}</tbody></table></div></div>
            </>
          )}
          {active==='Sales' && <div style={{padding:16}}><div className="box"><h3 style={{marginBottom:12}}>Sales</h3><table><thead><tr><th>ID</th><th>Customer</th><th>Amount</th><th>Status</th></tr></thead><tbody>{transactions.map(t=><tr key={t.id}><td>{t.id}</td><td>{t.customer}</td><td>₦{t.amount.toLocaleString()}</td><td>{t.status}</td></tr>)}</tbody></table></div></div>}
          {active==='Inventory' && <div style={{padding:16}}><div className="box"><h3>Inventory</h3><table><thead><tr><th>Product Name</th><th>Category</th><th>Stock Qty</th><th>Price</th><th>Stock Status</th><th>Action</th></tr></thead><tbody>{filtered.map(i=>{const c=getColor(i.stock); return <tr key={i.id}><td>{i.product}</td><td>{i.category}</td><td>{i.stock}</td><td>₦{i.price.toLocaleString()}</td><td><span style={{background:c.bg,color:c.c,padding:'4px 10px',borderRadius:20,fontSize:11}}>{getStatus(i.stock)}</span></td><td><button onClick={()=>setInventory(inventory.map(x=>x.id===i.id?{...x,stock:x.stock+10}:x))} style={{padding:'4px 8px',borderRadius:6}}>Restock</button></td></tr>})}</tbody></table></div></div>}
          ...
          {active==='AI Insights' && <div style={{padding:16}}><div className="box" style={{background:'#f0fdf4'}}><b>AI:</b> Indomie & Coke will spike this weekend +32% — restock Dettol before Friday.</div></div>}
